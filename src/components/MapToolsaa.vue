...
export default {
  ...
  methods:{
    handleMapToolsitemClick(e){ //行政区划导航、目录树管理、清屏的点击事件
    ...
    handleCommand(command) { //地图测量、更多功能的点击事件
        switch (command) {
            case 'distance':
                break;
            case 'area':
            ...
            case 'clear':
            case 'spacequery':
                this.initSpaceQuery(); //点击“空间查询”，开始初始化空间查询
                break;
            case 'morescreen':
                break;
            case 'swipanalyst':
    ...
    async initSpaceQuery(){
        const _self = this;
        const view = _self.$store.getters._getDefaultView;//获取到vuex的view
        //绘制面状区域
        ...
        //如果polygonGraphicLayer已存在，就把它删掉
        const resultLayer = view.map.findLayerById('polygonGraphicLayer');
        if (resultLayer) view.map.remove(resultLayer);
        //创建polygonGraphicLayer
        const graphicsLayer  = new GraphicsLayer({
          id:'polygonGraphicLayer',
          elevationInfo: { //覆盖在其他图层之上
              mode: 'on-the-ground',
          },
        });
        view.map.add(graphicsLayer);
        console.log('加载graphicsLayer')
        //定义polygonGraphicLayer的符号
        const polygonSymbol = {
          type:'simple-fill',
